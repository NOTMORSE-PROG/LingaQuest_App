...
export default function MapScreen() {
  // BUG 22 FIX: use hook so dimensions update on orientation/resize instead of stale module-load value
  const { width: sw } = useWindowDimensions();
  const CANVAS_H = Math.round(sw * 2.77); // proportional to design reference (1080 / 390)
  ...
  // BUG 6 FIX: show error screen instead of permanent spinner on failure
    ...
    <ScrollView className="flex-1 bg-ocean-deep" showsVerticalScrollIndicator={false}>
      ...
      <View style={{ width: sw, height: CANVAS_H + 60, position: "relative" }}>
        ...
        <Svg width={sw} height={CANVAS_H + 60} style={{ position: "absolute", top: 0, left: 0 }}>
          ...
          {/* Subtle wave lines — y positions proportional to CANVAS_H */}
          {[0.102, 0.241, 0.380, 0.519, 0.657, 0.796, 0.935].map((yf, i) => {
          ...
          <G transform={`translate(${(sw * 0.5 - 15).toFixed(0)}, ${(CANVAS_H * 0.248).toFixed(0)})`}>
          ...
          <G transform={`translate(${(sw * 0.08).toFixed(0)}, ${(CANVAS_H * 0.494).toFixed(0)})`}>
          ...
          <G transform={`translate(${(sw * 0.56).toFixed(0)}, ${(CANVAS_H * 0.611).toFixed(0)})`}>
          ...
          {/* Stars near final island — positions proportional to canvas */}
          {[[sw * 0.15, CANVAS_H * 0.806], [sw * 0.82, CANVAS_H * 0.778], [sw * 0.68, CANVAS_H * 0.852]].map(([sx, sy], i) => (
            <Path
              key={i}
...
function IslandNode({
  ...
  useEffect(() => {
    ...
    // BUG 15 FIX: cancel animations on unmount or when isCurrent goes false
    return () => {
