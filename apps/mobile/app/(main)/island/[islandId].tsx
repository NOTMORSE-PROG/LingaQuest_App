...
function FrozenOceanBackground({ width, height }: { width: number; height: number }) {
  ...
  return (
    <>
      ...
      {/* Animated ice floes */}
      <IceFloe x={width * 0.05} y={CANVAS_H * 0.76} w={60} h={18} delay={0} />
      <IceFloe x={width * 0.46} y={CANVAS_H * 0.81} w={40} h={12} delay={1} />
      <IceFloe x={width * 0.64} y={CANVAS_H * 0.73} w={80} h={20} delay={2} />
      <IceFloe x={width * 0.24} y={CANVAS_H * 0.87} w={35} h={10} delay={3} />
    </>
  );
}

// ─── Island 2: Speedy Purple Background ──────────────────────────────────────
function SpeedBolt({ y, w, delay, color }: { y: number; w: number; delay: number; color: string }) {
  const tx = useSharedValue(-w - 10);
  useEffect(() => {
    tx.value = withRepeat(
      withSequence(
        withTiming(-w - 10, { duration: 0 }),
        withTiming(SW + 10, { duration: 700 + delay * 120, easing: Easing.linear })
      ),
      -1, false
    );
...
function SpeedyBackground({ width, height }: { width: number; height: number }) {
  return (
    <>
      ...
      <SpeedBolt y={CANVAS_H * 0.15} w={80}  delay={0} color="rgba(142,68,173,0.22)" />
      <SpeedBolt y={CANVAS_H * 0.28} w={120} delay={1} color="rgba(180,100,220,0.18)" />
      <SpeedBolt y={CANVAS_H * 0.41} w={60}  delay={2} color="rgba(142,68,173,0.20)" />
      <SpeedBolt y={CANVAS_H * 0.55} w={100} delay={3} color="rgba(160,80,200,0.16)" />
      <SpeedBolt y={CANVAS_H * 0.68} w={80}  delay={1} color="rgba(142,68,173,0.22)" />
      <SpeedBolt y={CANVAS_H * 0.82} w={50}  delay={4} color="rgba(180,100,220,0.18)" />
    </>
  );
}
...
function FoggyBackground({ width, height }: { width: number; height: number }) {
  return (
    <>
      ...
      <FogDrift x={width * 0.00} y={CANVAS_H * 0.12} w={width * 0.65} h={60} delay={0} />
      <FogDrift x={width * 0.35} y={CANVAS_H * 0.30} w={width * 0.55} h={50} delay={1} />
      <FogDrift x={width * 0.05} y={CANVAS_H * 0.52} w={width * 0.70} h={70} delay={2} />
      <FogDrift x={width * 0.20} y={CANVAS_H * 0.70} w={width * 0.60} h={55} delay={3} />
      <FogDrift x={width * 0.00} y={CANVAS_H * 0.88} w={width * 0.75} h={65} delay={1} />
    </>
  );
}

// ─── Island 4: Ember / Ash Background ────────────────────────────────────────
function EmberFloat({ x, delay, color }: { x: number; delay: number; color: string }) {
  const y = useSharedValue(CANVAS_H + 10);
  const opacity = useSharedValue(0);

  useEffect(() => {
    y.value = withRepeat(
      withSequence(
        withTiming(CANVAS_H + 10, { duration: 0 }),
...
function EmberBackground({ width, height }: { width: number; height: number }) {
  return (
    <>
      ...
      <EmberFloat x={width * 0.12} delay={0} color="rgba(231,76,60,0.18)" />
      <EmberFloat x={width * 0.34} delay={2} color="rgba(200,80,40,0.15)" />
      <EmberFloat x={width * 0.55} delay={1} color="rgba(231,76,60,0.18)" />
      <EmberFloat x={width * 0.72} delay={3} color="rgba(245,130,80,0.14)" />
      <EmberFloat x={width * 0.88} delay={1} color="rgba(231,76,60,0.16)" />
    </>
  );
}
...
function ScatteredBackground({ width, height }: { width: number; height: number }) {
  return (
    <>
      ...
      <ScatterDrift char="7"  x={width * 0.08} startY={CANVAS_H * 0.75} angle={-18} delay={0} />
      <ScatterDrift char="B"  x={width * 0.25} startY={CANVAS_H * 0.60} angle={12}  delay={1} />
      <ScatterDrift char="?"  x={width * 0.45} startY={CANVAS_H * 0.82} angle={-8}  delay={2} />
      <ScatterDrift char="42" x={width * 0.62} startY={CANVAS_H * 0.50} angle={22}  delay={3} />
      <ScatterDrift char="C"  x={width * 0.78} startY={CANVAS_H * 0.70} angle={-15} delay={1} />
      <ScatterDrift char="!"  x={width * 0.90} startY={CANVAS_H * 0.88} angle={10}  delay={4} />
    </>
  );
}

// ─── Island 6: Story Fragments Background ─────────────────────────────────────
function ScrollDrift({ x, delay, rotation }: { x: number; delay: number; rotation: number }) {
  ...
  useEffect(() => {
    y.value = withRepeat(
      withSequence(
        ...
        withTiming(CANVAS_H + 40, { duration: 14000 + delay * 1200, easing: Easing.linear })
      ),
      -1, false
    );
    opacity.value = withRepeat(
...
function StoryBackground({ width, height }: { width: number; height: number }) {
  return (
    <>
      ...
      <ScrollDrift x={width * 0.08} delay={0} rotation={-14} />
      <ScrollDrift x={width * 0.28} delay={2} rotation={10}  />
      <ScrollDrift x={width * 0.50} delay={1} rotation={-7}  />
      <ScrollDrift x={width * 0.70} delay={3} rotation={16}  />
      <ScrollDrift x={width * 0.88} delay={1} rotation={-11} />
    </>
  );
}
...
function StormBackground({ width, height }: { width: number; height: number }) {
  return (
    <>
      ...
      <GoldFlicker x={width * 0.12} y={CANVAS_H * 0.08} delay={0} />
      <GoldFlicker x={width * 0.75} y={CANVAS_H * 0.22} delay={1} />
      <GoldFlicker x={width * 0.35} y={CANVAS_H * 0.45} delay={2} />
      <GoldFlicker x={width * 0.60} y={CANVAS_H * 0.60} delay={3} />
      <GoldFlicker x={width * 0.18} y={CANVAS_H * 0.75} delay={1} />
      <GoldFlicker x={width * 0.88} y={CANVAS_H * 0.85} delay={4} />
      {/* Animated echo rings */}
      <EchoRingBg cx={width * 0.50} cy={CANVAS_H * 0.35} delay={0} />
      <EchoRingBg cx={width * 0.38} cy={CANVAS_H * 0.65} delay={4} />
    </>
  );
}
    ...
    <SafeAreaView className="flex-1 bg-ocean-deep" edges={["top"]}>
      ...
      {cumulativeAccuracy !== null && (
        <View style={{ paddingHorizontal: 20, paddingBottom: 10 }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}>
            <Text style={{ color: "#9ca3af", fontSize: 10 }}>Island Average</Text>
            <Text style={{
              color: islandPassed ? "#4ade80" : cumulativeAccuracy >= 50 ? "#f5c518" : "#f87171",
              fontSize: 10, fontWeight: "700",
            }}>
              {cumulativeAccuracy}% {islandPassed ? "✓" : "/ 70% needed"}
            </Text>
          </View>
          <View style={{ height: 4, backgroundColor: "rgba(255,255,255,0.1)", borderRadius: 2, overflow: "hidden" }}>
            <View style={{
              ...
              backgroundColor: islandPassed ? "#4ade80" : cumulativeAccuracy >= 50 ? "#f5c518" : "#f87171",
              borderRadius: 2,
            }} />
          </View>
        </View>
      ...
      {showShardWarning && (
        <View style={{
          ...
          <CaptainSalita
            state="talking"
            dialogue={`Your voyage across this island scored ${cumulativeAccuracy}% — you need 70% to sail on. Replay any challenge to raise your average, sailor.`}
            size={110}
          />
        </View>
      )}
