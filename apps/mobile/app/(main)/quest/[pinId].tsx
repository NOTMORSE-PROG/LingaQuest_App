...
type Phase = "intro" | "listening" | "answering" | "result" | "pinComplete";
...
export default function QuestScreen() {
  ...
  function confirmExit() {
    if (phase === "intro" || phase === "pinComplete") { router.back(); return; }
    Alert.alert(
      "Abandon this pin?",
  ...
  const pinSortOrder: number = (pin as any).sortOrder ?? 0;
  const questBg =
    ...
    >
      ...
      {phase === "pinComplete" && (
        <View className="mt-8 items-center px-2">
          <Text className="text-5xl mb-4">{pinScore >= 70 ? "⭐" : "🎧"}</Text>
          <Text className="text-gold text-2xl font-bold text-center mb-2">
            Pin Complete
          </Text>
          <Text className="text-parchment text-4xl font-bold mb-1">{pinScore}%</Text>
          <Text
            style={{ color: pinScore >= 70 ? "#4ade80" : "#f87171", fontSize: 13, marginBottom: 24 }}
          >
            {pinScore >= 70 ? "✓ Above threshold" : "✗ Below 70% — score recorded"}
          </Text>
          <Text className="text-parchment/60 text-xs text-center mb-6 leading-5">
            {pinScore >= 70
              ? "Great listening, sailor. Onward."
              : "Your score has been saved. You can replay this pin later to raise your island average."}
          </Text>
          <TouchableOpacity
            onPress={() => router.back()}
            className="bg-gold rounded-xl px-10 py-4 w-full items-center"
          >
            <Text className="text-ocean-deep font-bold text-lg">Return to Island</Text>
          </TouchableOpacity>
        </View>
      )}
